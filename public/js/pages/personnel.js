					...
					success: function (data) {
							...
							Confirm('تایید', '45%', '', 'آیتم شما با کد پیگیری  ' + data_id + 'از سیستم حذف شد ', {
								tryAgain: {
									...
									btnClass: 'btn-' + 'green',
									action: function () {
										$(".alert-info").html("Successfully updated personnel!");
									}
								}
							}, 'green');
						},
...
function funcShow(e){
	let data_id = $(e).parent().parent().attr("data-id");
	window.open('/personnel/'+ data_id, '_blank');
}

function funcEdit(e){
	let data_id = $(e).parent().parent().attr("data-id");
	window.open('/personnel/'+ data_id + '/edit', '_blank');
}
...
$(document).ready(function () {
	...
	$("#btn-filter").click(function(){
		tt_personnel.ajax.reload();
	});
    tt_personnel = $('#tbl-personnel').on('preXhr.dt', function (e, settings, json, xhr) {}).DataTable(
		{
			...
			"columns": [			
				...
				{title: 'کد ملی', "name": 'mellicode', "data": 'mellicode'},
				{title: 'همراه 1', "name": 'mobile1', "data": 'mobile1'},
                {title: 'همراه 2', "name": 'mobile2', "data": 'mobile2'},				
				{title: 'آدرس', "name": 'address', "data": 'address'},
				{
					data: null, title: 'وضعیت', render: function (data, type, row) {
						return data.status == 0 ? "غیر فعال": "فعال";
					}
					, "visible": true
				},
				{
			...
			createdRow: function (row, data, dataIndex) {
				...
				$(row).attr("data-mellicode", data['mellicode']);
				$(row).attr("data-mobile", data['mobile1']);
