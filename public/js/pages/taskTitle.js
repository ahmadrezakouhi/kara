...
function funcDelete(e) {
    ...
    Confirm('توجه', '40%', 'BackToHome|5000', data_name + ' از لیست عنوان فعالیتها حذف شود؟ ', {
        BackToHome: {
...
function funcShow(e){
	let data_id = $(e).parent().parent().attr("data-id");
	window.open('/taskTitle/'+ data_id, '_blank');
}

function funcEdit(e){
	let data_id = $(e).parent().parent().attr("data-id");
	window.open('/taskTitle/'+ data_id + '/edit', '_blank');
}
...
$(document).ready(function () {
	...
	$("#btn-filter").click(function(){
		tt_tasktitle.ajax.reload();
	});
    tt_tasktitle = $('#tbl-task-title').on('preXhr.dt', function (e, settings, json, xhr) {}).DataTable(
		{
		...
		// ///////
		tt_tasktitle.on('draw', function() {
	
			tt_tasktitle.column(0, {
				search: 'applied',
				order: 'applied'
			}).nodes().each(function(cell, i) {
				
				cell.innerHTML = tt_tasktitle.page.len() * tt_tasktitle.page()+i+1;
			});
			}).draw();
		
		// ////
		$("#btnAddTaskTitle").click(function(){
			var required = ["#title"];
			required = checkRequired(required);
			if (required) {	
					$.ajax({
						url: "/taskTitle/addTaskTitle",
						type: 'POST',
						data: {
